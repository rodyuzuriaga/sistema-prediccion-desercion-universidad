...
def mostrar_formulas_matematicas():
    ...
    with st.sidebar.expander("Modelos de Supervivencia", expanded=False):
        st.markdown("**3. Cox Proportional Hazards**")
        st.latex(r'''
        h(t|X) = h_0(t)\exp(\beta^T X)
        ''')
        st.caption("*Cox, D.R. (1972)*")
        
        st.markdown("**4. Tasa de Retención**")
        st.latex(r'''
    ...
    with st.sidebar.expander("Análisis de Aprendizaje", expanded=False):
        st.markdown("**5. Bayesian Knowledge Tracing**")
        st.latex(r'''
        P(L_t|obs) = \frac{P(obs|L_t)P(L_t)}{P(obs)}
        ''')
        st.caption("*Corbett & Anderson (1994)*")
        
        st.markdown("**6. Entropía Académica**")
        st.latex(r'''
    ...
    with st.sidebar.expander("Modelos Predictivos", expanded=False):
        st.markdown("**9. Modelo IRT (2PL)**")
        st.latex(r'''
        P(\text{correcto}|\theta) = \frac{1}{1 + e^{-a_i(\theta - b_i)}}
        ''')
        st.caption("*Lord, F.M. (1980)*")
        
        st.markdown("**10. Modelo ARIMA**")
        st.latex(r'''
        \phi(B)(1-B)^d y_t = \theta(B)\epsilon_t
        ''')
        st.caption("*Box & Jenkins (1976)*")
    
    with st.sidebar.expander("Análisis del Perfil", expanded=False):
        st.markdown("**11. Eficiencia de Aprendizaje**")
        st.latex(r'''
        E = \frac{\Delta \text{score}}{\Delta \text{horas} + \epsilon}
        ''')
        st.caption("*Newell & Rosenbloom (1981)*")
        
        st.markdown("**12. Vulnerabilidad Financiera**")
        st.latex(r'''
